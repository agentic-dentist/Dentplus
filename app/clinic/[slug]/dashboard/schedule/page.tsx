...
export default function SchedulePage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Dentist/hygienist default to their own column
      if (staff.role === 'dentist' || staff.role === 'hygienist') {
        setActiveProvider(staff.id)
      }

      // Fetch providers via service-role API route — bypasses RLS for all roles
  ...
  // All unassigned appointments in this week
  const unassignedThisWeek = appointments.filter(a => !a.provider_id)
  const hasUnassigned = unassignedThisWeek.length > 0
  ...
  return (
    <>
      <style>{`
        ...
        .th-provider{padding:10px 12px;border-bottom:1px solid #F1F5F9;background:#FAFBFC;min-width:140px}
        ...
        .td-apt{padding:3px;border-top:1px solid #F8FAFC;vertical-align:top;min-width:140px}
        ...
        .ai-tag{display:inline-block;font-size:9px;font-weight:700;padding:1px 5px;border-radius:4px;background:#EFF6FF;color:#0284C7;margin-top:2px;letter-spacing:.3px;margin-right:3px}
        .ml-tag{display:inline-block;font-size:9px;font-weight:700;padding:1px 5px;border-radius:4px;background:#FDF4FF;color:#7C3AED;margin-top:2px;letter-spacing:.3px}
        ...
        <>
            ...
            <table className="sched-grid">
              <thead>
                <tr>
                    ...
                    visibleProviders.map((prov, pi) => {
                      ...
                      return (
                        <th key={`${day.toISOString()}-${prov.id}`} className="th-provider">
                          ...
                          <div style={{ color, fontSize: '11px', fontWeight: 500 }}>
                  ...
                  {/* Unassigned column — only shown when unassigned apts exist this week */}
                  {hasUnassigned && activeProvider === 'all' && days.map(day => (
              ...
              <tbody>
                {HOURS.map(hour => (
                  <tr key={hour}>
                    ...
                    {/* Unassigned column cells */}
                    {hasUnassigned && activeProvider === 'all' && days.map(day => {
                      const apt   = getApt(day, hour, null)
                      const color = apt ? (TYPE_COLOR[apt.appointment_type] || '#94A3B8') : null
                      return (
          ...
          <div className="legend">
            ...
            <div className="legend-item">
              <div style={{ width: '10px', height: '10px', borderRadius: '3px', background: '#FDF4FF', border: '1px solid #7C3AED', flexShrink: 0 }} />
              Matchmaker
            </div>
          </div>
        </>
