...
export default function SchedulePage() {
  ...
  return (
    <>
      <style>{`
        .page-title { font-size: 22px; font-weight: 700; color: #0F172A; letter-spacing: -0.3px; margin-bottom: 20px; }
        ...
        .day-header.today { background: #F0FDFA; }
        .day-name { letter-spacing: 0.5px; }
        .day-num { font-size: 20px; font-weight: 700; color: #0F172A; margin-top: 2px; }
        .day-num.today { color: #0F766E; }
        .day-apts { padding: 10px; min-height: 120px; }
        .apt-pill {
            ...
            return (
              <div key={day.toISOString()} className="day-col">
                ...
                <div className="day-apts">
                  {apts.length === 0 ? (
                    <div className="no-apts">No appointments</div>
                  ) : (
                    apts.map(apt => (
                      <div
                        key={apt.id}
                        className="apt-pill"
                        style={{
                          background: `${TYPE_COLOR[apt.appointment_type]}15`,
