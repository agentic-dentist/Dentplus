...
const PLANS = [
  {
    ...
    price: '$99',
    period: '/month',
    features: ['Up to 3 staff members', '200 patients', 'AI booking agent', 'Patient portal', 'Email support'],
    color: '#0EA5E9',
    id: 'starter'
  },
  {
    ...
    price: '$199',
    period: '/month',
    features: ['Up to 10 staff members', '1,000 patients', 'AI booking + waitlist agent', 'SMS confirmations', 'Insurance pre-check', 'Priority support'],
    color: '#6366F1',
    id: 'pro',
    popular: true
  },
  ...
  return (
    <>
      <style>{`
        ...
        .plan-info { }
        .plan-label { font-size: 11px; font-weight: 600; letter-spacing: 0.8px; text-transform: uppercase; color: #94A3B8; margin-bottom: 4px; }
        ...
        .plan-card { background: white; border-radius: 12px; border: 1px solid #E2E8F0; padding: 24px; position: relative; transition: box-shadow 0.15s; }
        .plan-card:hover { box-shadow: 0 4px 16px rgba(0,0,0,0.08); }
        .plan-card.popular { border-width: 2px; }
        .popular-tag { position: absolute; top: -10px; left: 50%; transform: translateX(-50%); padding: 3px 12px; border-radius: 20px; font-size: 10px; font-weight: 700; color: white; white-space: nowrap; letter-spacing: 0.5px; }
        .plan-price { font-family: 'Syne', sans-serif; font-size: 28px; font-weight: 700; color: #0F172A; margin-bottom: 4px; }
        .plan-price span { font-size: 14px; font-weight: 400; color: #94A3B8; }
        .plan-title { font-size: 16px; font-weight: 600; color: #0F172A; margin-bottom: 16px; }
        ...
        .plan-btn { width: 100%; padding: 10px; border-radius: 8px; font-size: 13px; font-weight: 500; font-family: 'DM Sans', sans-serif; cursor: pointer; border: 1.5px solid #E2E8F0; background: white; color: #475569; transition: all 0.15s; }
        .plan-btn:hover { background: #F8FAFC; }
        .plan-btn.primary { color: white; border-color: transparent; }
        .plan-btn.primary:hover { filter: brightness(0.92); }
        .coming-soon { text-align: center; padding: 32px; color: #94A3B8; font-size: 13px; margin-top: 16px; }
      `}</style>
      ...
      {!loading && sub && (
        <div className="current-plan">
          ...
          {sub.plan === 'trial' && (
            <div className="trial-badge">{trialDaysLeft} days left</div>
          )}
        </div>
      )}
      ...
      <div className="plans-grid">
        {PLANS.map(plan => (
          <div key={plan.id} className={`plan-card ${plan.popular ? 'popular' : ''}`} style={plan.popular ? { borderColor: plan.color } : {}}>
            {plan.popular && (
              <div className="popular-tag" style={{ background: plan.color }}>Most popular</div>
            )}
            ...
            <ul className="features">
              {plan.features.map(f => (
                <li key={f} style={{ '--check-color': plan.color } as React.CSSProperties}>{f}</li>
              ))}
            </ul>
            <button
